...
class _EditTransactionScreenState extends ConsumerState<EditTransactionScreen>
  ...
  // Track whether the account list has been loaded and pre-selection applied.
  bool _accountPreselected = false;
  ...
  void initState() {
    ...
    _tabController = TabController(
      length: 2,
      vsync: this,
      // Start on the correct tab for the existing type.
      initialIndex: _type == AppStrings.expense ? 0 : 1,
    );
      ...
      if (!_tabController.indexIsChanging) {
        setState(() {
          _type = _tabController.index == 0
              ? AppStrings.expense
              : AppStrings.income;
          // Clear category when switching type — income/expense categories
          // are different sets.
  ...
  // Same balance check as add_transaction_screen, but skips the check when
  // the user chose the same account as the original (balance already reflects
  // the original transaction, so effective delta is amount difference only).
  // For simplicity — and because the delete+create approach re-runs the full
  // balance logic anyway — we check against current balance the same way.
    ...
    try {
      await ref
          ...
          .updateTransaction(
            ...
            updated: CreateTransactionParams(
              amount: amount,
              type: _type,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Transaction updated!', style: GoogleFonts.poppins()),
  ...
  Widget build(BuildContext context) {
    ...
    // Pre-select account once the list loads.
    accountsAsync.whenData((accounts) {
      if (!_accountPreselected) {
        final match = accounts
            .where((a) => a.id == widget.transaction.accountId)
            .firstOrNull;
        if (match != null) {
          _accountPreselected = true;
          WidgetsBinding.instance.addPostFrameCallback(
            (_) => setState(() => _selectedAccount = match),
          );
        }
      }
    });

    // Pre-select category once the list loads.
...
// ── Supporting widgets (mirrors add_transaction_screen) ───────────────────────
